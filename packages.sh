# Dependencies for weblocks
# by Aaron Feng
# aaron.feng@gmail.com
# http://twitter.com/aaronfeng
PACKAGES_URL="http://cloud.github.com/downloads/aaronfeng/weblocks-install"
PACKAGES="
usocket-0.4.1.tar.gz \
trivial-timeout-0.1.5.tar.gz \
trivial-gray-streams-2008-11-02.tar.gz \
trivial-features-0.5.0.tar.gz \
tinaa-0.5.11.tar.gz \
split-sequence-20011114.1.tar.gz \
salza2-2.0.7.tar.gz \
s-xml-3.0.0.tar.gz \
s-sysdeps-1.0.0.tar.gz \
rt-1990.12.19.tar.gz \
rfc2388.tar.gz \
puri-1.5.1.tar.gz \
parenscript-20090216.tar.gz \
moptilities-0.3.13.tar.gz \
metatilities-base-0.6.6.tar.gz \
metatilities-0.6.18.tar.gz \
metabang-bind-0.7.4.tar.gz \
md5-1.8.5.tar.gz \
hunchentoot-1.0.0.tar.gz \
flexi-streams-1.0.7.tar.gz \
fare-utils-1.96.0.tar.gz \
fare-matcher.tar.gz \
f-underscore-0.1.0.tar.gz \
closer-mop-0.55.0.tar.gz \
cl-ssl-2008-11-04.tar.gz \
cl-who-0.11.1.tar.gz \
cl-prevalence-3.0.0.tar.gz \
cl-ppcre-2.0.1.tar.gz \
cl-json-0.4.0.tar.gz \
cl-fad-0.6.2.tar.gz \
cl-containers-0.11.5.tar.gz \
cl-cont-0.3.7.tar.gz \
cl-base64-3.3.3.tar.gz \
chunga-1.0.0.tar.gz \
cffi-0.10.5.tar.gz \
bordeaux-threads-0.6.0.tar.gz \
babel-0.3.0.tar.gz \
alexandria-2008-07-29.tar.gz \
anaphora-0.9.3.tar.gz \
asdf-system-connections-0.1.tar.gz \
slime-2010-02-14.tar.gz \
linedit-0.16.1.tar.gz \
osicat-0.6.0.tar.gz \
terminfo-1.3.tar.gz \
uffi-1.6.2.tar.gz \
lift-1.7.0.tar.gz \
html-template-0.9.1.tar.gz
" 
